...
# =========================================================================
# [시나리오 2 피드백 반영] 반도체를 모르는 사용자를 위한 친절한 설명 가이드 추가
# =========================================================================
st.markdown("---")
st.subheader("💡 시뮬레이터 이용 가이드 (동료 피드백 반영)")

with st.expander("❓ 현재 저항과 전도도(학습 강도)는 왜 변하나요?"):
    st.markdown("""
    * **전기 자극 주기 버튼을 누르면:** 가상의 전류가 흐르면서 멤리스터 소자의 저항이 **비선형적으로 팍팍 감소**합니다. 저항이 낮아질수록 전류가 더 잘 흐르게 되는데, 이를 **전도도(Conductance)가 높아졌다**고 하며 화면의 소자 색상이 진한 빨간색으로 변합니다. 인공지능이 데이터를 강하게 '학습'한 상태를 뜻합니다.
    * **가만히 놔두면:** 시간이 흐름에 따라 저항이 서서히 다시 원래 상태(100 Ω)로 돌아가며 색이 연해집니다. 이는 인간의 뇌가 시냅스 자극이 없으면 기억을 잃어버리는 **'자연 망각(Forgetting) 특성'**을 소프트웨어 수식으로 재현한 것입니다.
    """)
with col2:
    st.header
